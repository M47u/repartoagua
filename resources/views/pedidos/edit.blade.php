...
@section('title', 'Editar Pedido - RepartoAgua')
        ...
        <div class="col-md-10">
            <div class="card">
                <div class="card-header bg-warning">
                    <h5 class="mb-0"><i class="bi bi-pencil"></i> Editar Pedido</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('pedidos.update', $pedido->id) }}" method="POST" id="formPedido">
                        @csrf
                        @method('PUT')
                                ...
                                <select class="form-select @error('producto_id') is-invalid @enderror" 
                                    ...
                                    @foreach($productos ?? [] as $producto)
                                        <option value="{{ $producto->id }}" data-precio="{{ $producto->precio }}" 
                                            {{ old('producto_id', $pedido->producto_id) == $producto->id ? 'selected' : '' }}>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                ...
                                <input type="number" class="form-control @error('cantidad') is-invalid @enderror" 
                                       id="cantidad" name="cantidad" value="{{ old('cantidad', $pedido->cantidad) }}" min="1" required>
                                @error('cantidad')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" step="0.01" class="form-control @error('precio_unitario') is-invalid @enderror" 
                                           id="precio_unitario" name="precio_unitario" value="{{ old('precio_unitario', $pedido->precio_unitario) }}" required>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" step="0.01" class="form-control" 
                                           id="total" name="total" value="{{ old('total', $pedido->total) }}" readonly>
                                </div>
                            </div>
                        </div>
                                ...
                                <select class="form-select @error('ruta_id') is-invalid @enderror" 
                                    ...
                                    @foreach($rutas ?? [] as $ruta)
                                        <option value="{{ $ruta->id }}" 
                                            {{ old('ruta_id', $pedido->ruta_id) == $ruta->id ? 'selected' : '' }}>
                                            {{ $ruta->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" class="form-control @error('fecha_entrega') is-invalid @enderror" 
                                       id="fecha_entrega" name="fecha_entrega" value="{{ old('fecha_entrega', $pedido->fecha_entrega) }}">
                                @error('fecha_entrega')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
